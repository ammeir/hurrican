...
////		Protokoll.WriteText( false, " Sprite Textur(en) �brig !\n" );
//    }
}
                                     ...
                                     int xfc,  int yfc)
{
    ...
    sprintf_s(Temp, "%s/textures/%s", g_storage_ext, Filename);

#if defined(USE_ETC1)
    if (DirectGraphics.SupportedETC1 == true) {
        sprintf_s( compresstex, "%s/textures/etc1/%s.pkm", g_storage_ext, Filename );
        if (FileExists(compresstex))
        {
            sprintf_s( Temp, "%s/textures/etc1/%s", g_storage_ext, Filename );
            goto loadfile;
        }
    }
#endif
...
#if defined(USE_PVRTC)
    if (DirectGraphics.SupportedPVRTC == true) {
        sprintf_s( compresstex, "%s/textures/pvr/%s.pvr", g_storage_ext, Filename );
        if (FileExists(compresstex))
        {
            sprintf_s( Temp, "%s/textures/pvr/%s", g_storage_ext, Filename );
            goto loadfile;
        }
    }
#endif
...
// alle vier Eckpunkte anders einFürben
// --------------------------------------------------------------------------------------
...
void RenderCircle(float x, float y, float r, D3DCOLOR col)
{
	...
	D3DXVECTOR2 p1, p2;
	for (float i = 0; i < PI_mul2; i += step)
	{
