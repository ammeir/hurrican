...
bool TileEngineClass::LoadLevel(char Filename[100])
{
    ...
    sprintf_s(Temp, "%s/levels/%s", g_storage_ext,  Filename);
    if (FileExists(Temp))
        goto loadfile;
...
loadfile:
    ...
    for(i=0; i<LEVELSIZE_X; i++)
        for(int j=0; j<LEVELSIZE_Y; j++)
        {
            ...
            // damit man nicht immer auf alle vier möglichen Fl�ssigkeiten checken muss,
			...
			// PSVITA TWEAK
			if (TileAt(i, j).Block & BLOCKWERT_WASSERFALL)
				IsWaterfallLevel = true;
        }
...
// Vertices gefällt, die das selbe Tileset verwenden, bis ein anderes Tileset
...
void TileEngineClass::DrawWater(void)
{
	...
	static VERTEX2D wf1_tiles[32*24*6];
	static VERTEX2D wf2_tiles[32*24*6];
	int k = 0;
	int k2 = 0;
    ...
    yScreen = (float)(-yTileOffs) + RenderPosY * TILESIZE_Y;
    {
		...
		if (k > 0)
        {
            DirectGraphics.SetTexture(Wasserfall[0].itsTexIdx);
            DirectGraphics.RendertoBuffer (D3DPT_TRIANGLELIST, k/3, &wf1_tiles[0]);
			DirectGraphics.SetTexture(Wasserfall[1].itsTexIdx);
            DirectGraphics.RendertoBuffer (D3DPT_TRIANGLELIST, k2/3, &wf2_tiles[0]);
        }
    }
...
// --------------------------------------------------------------------------------------
// Die R�nder bei den Overlay Tiles der zerstörbaren Wände
...
// zerst�ren und true zurückliefern, damit der Schuss ebenfalls gelöscht wird
// --------------------------------------------------------------------------------------
...
// Jedes Tile hat an allen vier Ecken die selbe Farbe -> Keine überg�nge -> Scheisse
// --------------------------------------------------------------------------------------
...
// entsprechend der umliegenden Tiles -> smoothe überg�nge -> Geilomat!
// --------------------------------------------------------------------------------------
