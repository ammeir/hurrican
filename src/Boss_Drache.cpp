...
GegnerDrache::GegnerDrache(int Wert1, int Wert2, bool Light)
{
    ...
    // Value2 != 0? Dann Starten die Spieler auf dem R�cken des Drachen!
    if (Value2 != 0)
    {
...
void GegnerDrache::DoKI(void)
{
            ...
            Destroyable == true)
    {
        ...
        // Alle �brigen Mini-Drachen zerst�ren
        GegnerClass* pTemp;
        pTemp = Gegner.pStart;
        while(pTemp != NULL)
        {
    ...
    case GEGNER_WARTEN:
    {
        if (KieferWinkel > PI_dev2)
            KieferWinkel -= 5.0f SYNC;
        else
            KieferWinkel = PI_dev2;
    ...
    // Ausw�hlen, welche Aktion gemacht wird
    case GEGNER_AUSWAHL:
    {
        HeadLocked = false;
        ...
        switch(newmove)
        {
        // Spieler überfliegen und dabei eine Aktion ausf�hern?
        case 0 :
        {
            ...
            // Handlung festlegen, was er w�hrend dem überfliegen macht
            int j = rand()%2;
    ...
    case GEGNER_LAUFEN:
    {
        ...
        switch (Attack)
        {
        // Feuerbölle schiessen
        case GEGNER_SCHIESSEN :
        {
            KieferWinkel  += 0.75f SYNC;
            while (KieferWinkel > PI_mul2)
                KieferWinkel -= PI_mul2;
        ...
        // Am Rand angekommen? Dann wieder neue Aktion w�hlen
        if ((xSpeed > 0.0f &&
                xPos > Value1 + 1000.0f) ||
                (xSpeed < 0.0f &&
                 xPos < Value1 - 500))
