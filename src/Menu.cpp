...
//    int joy_idx = Player[0].JoystickIndex;
    ...
    for (int i=0; i<MAX_STARS; i++)
    {
        ...
        if (Stars[i].Count > PI_mul2)
            Stars[i].Count -= PI_mul2;
    }
...
#ifdef PSVITA // On psvita, also allow the Dpad buttons to control the menu
                if (DirectInput.Joysticks[joy_idx].JoystickButtons[VITA_PAD_RIGHT]) {
                    joy_right = true;
                } else if (DirectInput.Joysticks[joy_idx].JoystickButtons[VITA_PAD_LEFT]) {
                    joy_left  = true;
                } else if (DirectInput.Joysticks[joy_idx].JoystickButtons[VITA_PAD_DOWN]) {
                    joy_down  = true;
                } else if (DirectInput.Joysticks[joy_idx].JoystickButtons[VITA_PAD_UP]) {
                    joy_up  = true;
                }
#endif // PSVITA
#endif //GCW
...
#ifdef PSVITA  
			// FIXME: Do nothing because Vita crashes when pressing Quit (in SDL_Quit).
            if (AktuellerPunkt == MENUPUNKT_END)
               break;
#endif
        ...
        if (selected)
        {
            ...
            if (AktuellerPunkt < MAX_SAVEGAMES)
            {
                ...
                */
                ...
                if (Datei == NULL)
                {
					//PSV_DEBUG("Failed to open save game file: %s", Name);
                }
