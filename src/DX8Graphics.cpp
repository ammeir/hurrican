...
#else
    actual_w = SCREENWIDTH;//Screen->w; // PSVITA
    actual_h = SCREENHEIGHT;//Screen->h; // PSVITA
#endif
...
bool DirectGraphicsClass::SetDeviceInfo(void)
{
    ...
    // Globale Variable mit dem tats�chlichen BackBuffer füllen
    ...
    sprintf_s(vert, "%s/shaders/%s/shader_color.vert", g_storage_ext, glsl_version);
    sprintf_s(frag, "%s/shaders/%s/shader_color.frag", g_storage_ext, glsl_version);
    ...
    sprintf_s(vert, "%s/shaders/%s/shader_texture.vert", g_storage_ext, glsl_version);
#if defined(USE_ETC1)
    if (SupportedETC1 == true) {
        sprintf_s(frag, "%s/shaders/%s/shader_etc1_texture.frag", g_storage_ext, glsl_version);
    } else {
#endif
        sprintf_s(frag, "%s/shaders/%s/shader_texture.frag", g_storage_ext, glsl_version);
#if defined(USE_ETC1)
    }
#endif
...
#else
    WindowView.w = SCREENWIDTH;//Screen->w; // PSVITA
    WindowView.h = SCREENHEIGHT;//Screen->h; // PSVITA
#endif
