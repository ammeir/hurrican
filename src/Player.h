...
#define	PLAYER_MAXJUMP			124.0f						// H�chster Sprung in Pixeln
...
#define PLAYER_ICESSLOWDOWN		0.995f						// Geschwindigkeits�nderung bei "Auslaufen" auf dem Eis
...
#define PLAYER_BLITZ_START		4.0f						// Zeit die man ben�tigt um Blitzen zu k�nnen
#define PLAYER_BEAM_MAX			160.0f						// Maximal aufladbare Beamst�rke
#define PLAYER_SHOTDELAY		1.8f						// Verz�gerung beim normalen schiessen
...
#define PISSEN   		16				// Der Spieler hat zu lange gewartet, und jetzt muss der arme Hurri �belst strullen (Auch als "Charles Bronson"-Animation bekannt)
...
private:
    ...
    // die Funktion aufrufen m�ssen, sondern nur EINMAL pro Frame
    VERTEX2D				v1, v2, v3, v4;			// Vertices zum Blitz rendern
    VERTEX2D				TriangleStrip[4];		// Strip f�r ein Sprite
    VERTEX2D				strahlen[NUM_COOL_LIGHTNINGS + 1][(30) * 2 + 1]; // F�r den Blitz
    float					AnimCount;				// Wann ist n�chste Animation ?
    ...
    float					changecount;			// z�hler, wann die blitze sich �ndern
...
public:
    ...
    bool					AlreadyDrawn;
    int						SoundOff;
    bool					Aktion[MAX_AKTIONEN];			// Array mit Tasten f�r Aktionen
    int						AktionKeyboard[MAX_AKTIONEN];	// Array mit Tasten  f�r Aktionen
    int						AktionJoystick[MAX_AKTIONEN];	// Array mit Buttons f�r Aktionen
    ...
    RECT					CollideRect;			// Rechteck f�r die Kollision
    GegnerClass				*AufPlattform;			// Auf einem Lift etc ?
    bool					InLiquid;				// Spieler in Einer Fl�ssigkeit?
    ...
    float					GameOverTimer;			// Timer f�r GameOver ?
    float					ShotDelay;				// Schuss Verz�gerung
    float					AutoFireCount;			// Wieviel 3er Autofire noch ?
    float					AutoFireExtra;			// Wieviel Sekunde Autofire Extra noch ?
    float					RiesenShotExtra;		// Spieler hat grad die Mega riesen Power Sch�sse ?
    ...
    float					xadd,    yadd;			// F�r das weiche fliegen beim "FlugSack-Reiten" :P
    float					BeamX, BeamY;			// Position der Waffenm�ndung, f�r die Partikel beim Beam laden
    float					BeamCount;				// Counter f�r Auflad-Partikel
    float					AutoScrollspeed;		// Mit welcher Speed scrollen ?
    float					JumpySave;				// Speichern der Koordinaten, falls
    float					JumpxSave;				// der Spieler wo runterf�llt
    float					JumpStart;				// ypos des Startpunktes beim Sprung
    float					JumpAdd;				// Dazuz�hlen beim wieder runterfallen
    float					SmokeCount;				// Counter f�r das Rauchen beim FlugSack-Fliegen
    float					BlitzWinkel;			// Aktuelle Richtung des Blitzes
    float					BlitzCount;				// Z�hler f�r die Blitzanimation
    ...
    int						BlocksThisLevel;		// Anzahl gefundener Powerbl�cke in diesem Level
    int						BlocksFullGame;			// Anzahl gefundener Powerbl�cke im ganzen Spiel
    int						Lives;					// Wieviele Leben noch ?
    int						SelectedWeapon;			// gew�hlte Waffe
    int						CurrentWeaponLevel[4];  // Aktuelles Level		 der Waffen 1-4
//		int						CollectedPowerUps[4];	// Gesammelt Powerups    der Waffen 1-4
//		int						NextWeaponLevel[4];		// N�chste   Waffenstufe der Waffen 1-4
    int						BlitzLength;			// L�nge des Blitzes
    ...
    float					BronsonCounter;			// Counter, wann der Hurri anf�ngt zu pullern =)
    ...
    float					JoystickSchwelle;		// Schwellenwert f�r Analog Stick
    int						ControlType;
    int						JoystickMode;
    ...
    DirectGraphicsSprite	PlayerPiss;				// Grafiken f�r das Pissen, wenn man lange nix macht =)
    DirectGraphicsSprite	PlayerRide;				// Grafiken f�r das Reiten auf dem FlugSack
    DirectGraphicsSprite	PlayerRad;				// Grafiken f�r das Rad
    ...
    bool GetPlayerInput(void);						// Eingabeger�te abfragen
    ...
    void RunPlayerExit		(void);					// Spieler l�uft grad ins Exit
