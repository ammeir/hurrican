...
#if defined (PLATFORM_SDL) && !defined(PSVITA)
#include <EGL/egl.h>
#if defined (USE_GL2)
#include <GLES2/gl2.h>
...
// false = Spiel so fl�ssig wie möglich laufen lassen
...
void FillCommandLineParams( int argc, char* args[] )
{
    ...
    CommandLineParams.VSync = true; // PSVITA: settings this to false improves performance but brings tear to tiles.
...
#elif defined (USE_SAVE_PATH)
		g_save_ext = (char*)malloc(strlen(USE_SAVE_PATH) + 1);
        strcpy_s(g_save_ext, USE_SAVE_PATH);
        // Attempt to locate the dir
        if (!FindDir(g_save_ext)){
            Protokoll.WriteText( false, "ERROR: Failed to locate save directory %s\n", g_save_ext );
			Protokoll.WriteText( false, "\tUsing '.' folder as fallback.\n" );
            g_save_ext = (char*)malloc(strlen(".") + 1);
            strcpy_s(g_save_ext, ".");
        }
