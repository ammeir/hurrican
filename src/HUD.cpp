...
//	int		 StateOff;
	...
	static VERTEX2D weaponpunkt_vertices[240]; // Check the limit
	static VERTEX2D weaponrahmen_vertices[240]; // Check the limit
	static VERTEX2D hudfontbig_vertices[240]; // Check the limit
	int weaponpunkt_ind = 0;
	int weaponrahmen_ind = 0;
	int hudfontbig_ind = 0;
	...
	if (weaponpunkt_ind > 0)
    {
        DirectGraphics.SetTexture(WeaponPunkt.itsTexIdx);
        DirectGraphics.RendertoBuffer (D3DPT_TRIANGLELIST, weaponpunkt_ind/3, &weaponpunkt_vertices[0]);
    }

    /*
    	 // Balken anzeigen, wieviele PowerUps noch zum nächsten Waffenlevel ben�tigt werden
    	 for (i=0; i<4; i++)
    	 {
    ...
    */
	...
	if (weaponrahmen_ind > 0)
    {
        DirectGraphics.SetTexture(WeaponRahmen.itsTexIdx);
        DirectGraphics.RendertoBuffer (D3DPT_TRIANGLELIST, weaponrahmen_ind/3, &weaponrahmen_vertices[0]);
    }
	...
	if (hudfontbig_ind > 0)
    {
        DirectGraphics.SetTexture(HUDFontBig.itsTexIdx);
        DirectGraphics.RendertoBuffer (D3DPT_TRIANGLELIST, hudfontbig_ind/3, &hudfontbig_vertices[0]);
    }
...
// --------------------------------------------------------------------------------------
// Alles machen, was da HUD betrifft, sprich, werte �ndern und dann anzeigen
// --------------------------------------------------------------------------------------
