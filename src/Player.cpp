...
void PlayerClass::DoStuffWhenDamaged(void)
{
    ...
    else
    {
        ...
        case 0:
        {
			...
			PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, FUNKE);
			PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, FUNKE);
			//PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, FUNKE); // PSVITA TWEAK
			//PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, FUNKE);
        ...
        case 1:
        {
			...
			PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, LONGFUNKE);
			PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, LONGFUNKE);
			//PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, LONGFUNKE); // PSVITA TWEAK
			//PartikelSystem.PushPartikel(x + rand()%4, y + rand()%4, LONGFUNKE);
    ...
    // Rauchz�hler runterzählen
    //
    if (smokecount > 0.0f)
        smokecount -= 1.0f SYNC;
    else
    {
        smokecount = 1.6f;//0.8f; PSVITA TWEAK
...
void PlayerClass::CheckForExplode(void)
{
            ...
            GameOverTimer  == 0.0f)
    {
		...
		int ypos_ = ypos + 10;
        //for (int i = 0; i < 3; i++)
        PartikelSystem.PushPartikel (xpos_ + rand () % 50, ypos_ + rand () % 50, SPLITTER);
		PartikelSystem.PushPartikel (xpos_ + rand () % 50, ypos_ + rand () % 50, SPLITTER);
		PartikelSystem.PushPartikel (xpos_ + rand () % 50, ypos_ + rand () % 50, SPLITTER);

        //for (int i = 0; i < 10; i++)
        PartikelSystem.PushPartikel (xpos_ + rand () % 50,ypos_ + rand () % 50, SPIDERSPLITTER);
		PartikelSystem.PushPartikel (xpos_ + rand () % 50,ypos_ + rand () % 50, SPIDERSPLITTER);
		PartikelSystem.PushPartikel (xpos_ + rand () % 50,ypos_ + rand () % 50, SPIDERSPLITTER);
		PartikelSystem.PushPartikel (xpos_ + rand () % 50,ypos_ + rand () % 50, SPIDERSPLITTER);
		PartikelSystem.PushPartikel (xpos_ + rand () % 50,ypos_ + rand () % 50, SPIDERSPLITTER);
...
void PlayerClass::AnimatePlayer(void)
{
            ...
            Handlung != DREHEN)
    {
        ...
        // Rechts/Links gedrückt ? stehenbleiben, da man ja nich r�ckw�rts UND vorw�rts gehen kann =)
        ...
        // nur, wenn keine Decke über dem Spieler (wenn er grad mit ner Plattform wo drunter durchf�hrt z.B.)
        if (Handlung == DUCKEN)
        {
        ...
        if (Handlung == BEAMLADEN)
        {
            ...
            // Beam aufladen. Je Länger der Blitz desto schneller l�dt der Beam
...

        {
            ...
            // Aufh�ren zu kullern und zurückverwandeln, wenn man springt oder keine Energie mehr hat
                    ...
                    Handlung == DREHEN)
            {
                ...
                if (SmokeCount < 0.0f)
                {
                    SmokeCount += 0.3f;//0.1f; // PSVITA TWEAK
...
void PlayerClass::DoPlattformStuff(void)
{
    ...
    // Fahrstuhl über Boden gefahren? Dann bleibt der Spieler daran h�ngen
    if (bu & BLOCKWERT_WAND ||
            bu & BLOCKWERT_PLATTFORM)
    {
        AufPlattform = NULL;
...
void PlayerClass::MovePlayer(void)
{
    ...
    // R�nder für gelockten Screen prüfen
    if (TileEngine.Zustand != ZUSTAND_SCROLLBAR)
    {
        if (xpos < TileEngine.XOffset)	 xpos = float(TileEngine.XOffset);
...
{
    int			DrawLength;			// Länge des Blitze mit ber�cksichtigten Wänden im Weg
    GegnerClass  *pEnemy;			// Für die Blitz/Gegner Kollision

    DrawLength = BlitzLength-1;		// Vom Maximum ausgehen (das wird später "gek�rzt")
...
// BeamBlitz aufladen
//
// Der Beam l�dt sich langsam auf, je nach BlitzLänge unterschiedlich schnell
// --------------------------------------------------------------------------------------
